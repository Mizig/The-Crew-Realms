...
public class FameStats
{
    ...
    public int CalculateTotal(XmlData data, Account acc, Char chr, int baseFame, out bool firstBorn)
    {
        ...
        if ((double)GodKills / (GodKills + MonsterKills) > 0.25)   //Slayer of the Gods
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        ...
        if ((double)ShotsThatDamage / Shots > 0.2)     //Accurate
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        if ((double)ShotsThatDamage / Shots > 0.4)      //Sharpshooter
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        if ((double)ShotsThatDamage / Shots > 0.5)     //Sniper
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        if (TilesUncovered > 100000)       //Explorer
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.05;
        }
        if (TilesUncovered > 400000)       //Cartographer
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.05;
        }
        if (LevelUpAssists > 25)           //Team Player
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        if (LevelUpAssists > 100)          //Leader of Men
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
        if (QuestsCompleted > 500)         //Doer of Deeds
        {
            bonus = Math.Floor(bonus) + (baseFame + Math.Floor(bonus)) * 0.1;
        }
