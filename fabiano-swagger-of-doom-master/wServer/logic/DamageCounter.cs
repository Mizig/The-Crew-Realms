...
namespace wServer.logic
{
    public class DamageCounter
    {
        ...
        public void Death(RealmTime time)
        {
            ...
            if (totalPlayer != 0)
            {
                float totalExp = totalPlayer*((float) enemy.ObjectDesc.MaxHP/5f)*(enemy.ObjectDesc.ExpMultiplier ?? 1);
                float lowerLimit = totalExp/totalPlayer*0.5f;
                int lvUps = 0;
                foreach (Tuple<Player, int> i in eligiblePlayers)
                {
                    float playerXp = totalExp*i.Item2/totalDamage;

                    float upperLimit = i.Item1.ExperienceGoal*8.0f;
                    if (i.Item1.Quest == enemy)
                        upperLimit = i.Item1.ExperienceGoal*8.0f;
